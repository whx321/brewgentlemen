	...
	if (!PAYPAL.apps.MiniCart) {
		...
		PAYPAL.apps.MiniCart = (function () {
			...
			var _addCSS = function () {
				...
				css.push('#' + name + ' li .remove { position:absolute; top:9px; right:60px; width:14px; height:14px; background:url(' + config.assetURL + 'img/minicart_sprite.png) no-repeat -134px -4px; border:0; cursor:pointer; } ');
				css.push('#' + name + ' p { margin:0; padding:0 0 0 20px; background:url(' + config.assetURL + 'img/minicart_sprite.png) no-repeat; font-size:13px; font-weight:bold; } ');
				css.push('#' + name + ' p:hover { cursor:pointer; } ');
				css.push('#' + name + ' p input { float:right; margin:4px 0 0; padding:1px 4px; text-decoration:none; font-weight:normal; color:#333; border:1px solid #d5bd98; -moz-border-radius:2px; -webkit-border-radius:2px; border-radius:2px; } ');	
			...
			var _checkout = function (e) {
				...
				//document.getElementById("PPMiniCart").getElementsByTagName("form")[0].getElementsByTagName("p")[0].getElementsByTagName("input")[0].className += "processing-order";
			...
			minicart.show = function (e) {
				var from = parseInt(minicart.UI.cart.offsetTop, 10) - 65, /* Zo change - subtract 65 (header height) */
					to = 0,
					events = config.events,
			...
			minicart.hide = function (e, fully) {
				var ui = minicart.UI,
					...
					from = parseInt(cartEl.offsetTop, 10) - 65, /* Zo change - subtract 65 (header height)  */
					events = config.events,
